  ...
  for frequency in board.list_frequency_chars() {
    ...
    for i in maps {
      for k in maps {
        ...
        antinode_list.push(*i);
        let delta:Vector = *i - *k;
        let mut sum_option:Option<Point> = *i + delta;
        while sum_option != None {
          let mut sum = sum_option.unwrap();
          antinode_list.push(sum);
          sum_option = sum + delta;
          if sum_option != None {
            sum = sum_option.unwrap();
            if !board.on_board(sum) {
              break;
            }
          }
        }
